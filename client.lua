...
-- Trigger manually to apply police outfit
RegisterNetEvent('qbx_job_items:applyPoliceOutfit', function()
    Wait(500)

    -- Save current appearance BEFORE uniform change
    ...
    if success then
        ...
        lib.notify({
            ...
            description = "Could not apply police outfit. Check illenium-appearance setup.",
            type = "error"
        })
    end
end)

-- Restore civilian appearance only when leaving police and becoming unemployed
    ...
    if newJob == "unemployed" and grade == 0 and currentJob == "police" then
        if prevAppearance then
            ...
            lib.notify({
                ...
                description = "Restored your previous outfit.",
                type = "info"
            })
        else
            lib.notify({
                title = "Outfit Not Saved",
                description = "No civilian clothing stored to restore.",
                type = "error"
            })
        end
    end
